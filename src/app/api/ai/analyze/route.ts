...
export async function POST(request: NextRequest) {
  try {
    const { code, language, userId, operation = 'bug_detection', model } = await request.json()
    ...
    // Make request to OpenRouter for bug detection
    const completion = await OpenRouterService.bugDetection(code, language, {
      model: model,
      temperature: 0.4,
      max_tokens: 3000
    })
    ...
    await Promise.all([
      ...
      UsageService.logUsage(userId, operation, actualTokensUsed, {
        ...
        complexity: actualTokensUsed > 1500 ? 'complex' : actualTokensUsed > 750 ? 'medium' : 'simple',
        responseTime,
        success: true,
